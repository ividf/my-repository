...
async function validate(accountData) {
  const schema = Joi.object({
    ...
    nick: Joi.string().max(255).required(),
...
async function createAccount(req, res) {
  ...
  try {
    ...
    return res.status(400).send({
      message: `Debes introducir obligatoriamente un NOMBRE, un NICK y un EMAIL que no excedan de los 255 caracteres y una CONTRASEÑA que sea alfanumérica y tenga entre 3 y 30 caracteres`,
    });
  }
  ...
  try {
    ...
    if (e.code === "ER_DUP_ENTRY") {
      return res.status(409).send({
        message: `La solicitud no pudo ser procesada, ya existe otra persona con ese NICK y/o EMAIL`,
      });
    }
